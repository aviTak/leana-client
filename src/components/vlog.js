...
import { getVlogsQuery } from '../queries/queries';
import { deleteVlogMutation } from '../queries/mutations';
...
class Vlog extends React.Component {
    ...
    submitForm(e){ 
        ...
        var data = this.props.getVlogsQuery;
    ...
    displayGrid(){ 
        ...
        const data = this.props.getVlogsQuery;
        ...
        if(mat.length !== 0){
            ...
            let a = mat.map(asset => (
                ...
                <tr key = {asset.id}>
                    ...
                    <td className = 'hover one'><Link style={{textDecoration: 'none'}} to = {'/vlog/title/' + asset.id}><div id={asset.id} style = {{width: '100%'}}>{asset.title}</div></Link></td>
        ...
        return(
            <div className='main__show home__box all__box' id='testScroll' onScroll = { this.handleScroll }>
                <div className='home__main'>
                    ...
                    <Link to='/vlog/add' style={style}><div className = 'new__blog'>
...
export default compose(
    ...
    graphql(deleteVlogMutation, {name: 'deleteVlogMutation'})
