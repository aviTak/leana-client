...
import { deleteMemberMutation } from '../queries/mutations';
...
class Team extends React.Component {
    ...
    componentWillUnmount(){
        try{
            document.getElementById('team').classList.remove('selected');
            clearTimeout(time);
        } catch(e){}
    }
    ...
    displayGrid(){ 
        ...
        const data = this.props.getMembersQuery;
        ...
        if(mat.length !== 0){
            ...
            let a = mat.map(asset => (
                ...
                <tr key = {asset.id}>
                    ...
                    <td className = 'hover one'><Link style={{textDecoration: 'none'}} to = {'/team/title/' + asset.id}><div id={asset.id} style = {{width: '100%'}}>{asset.name}</div></Link></td>
    ...
    deletePost(id){
        ...
        this.props.deleteMemberMutation({
        ...
        return(
            <div className='main__show home__box all__box' id='testScroll' onScroll = { this.handleScroll }>
                <div className='home__main'>
                    ...
                    <Link to='/team/add' style={style}><div className = 'new__blog'>
