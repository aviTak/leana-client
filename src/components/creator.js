...
import { getCreatorsQuery } from '../queries/queries';
import { deleteCreatorMutation } from '../queries/mutations';
...
class Creator extends React.Component {
    ...
    componentDidMount(){
        try{
            document.getElementById('creators').classList.add('selected');
        } catch(e){}
    ...
    componentWillUnmount(){
        try{
            document.getElementById('creators').classList.remove('selected');
            clearTimeout(time);
        } catch(e){}
    }
    ...
    displayGrid(){ 
        ...
        const data = this.props.getCreatorsQuery;
        ...
        if(mat.length !== 0){
            ...
            let a = mat.map(asset => (
                ...
                <tr key = {asset.id}>
                    ...
                    <td className = 'hover one'><Link style={{textDecoration: 'none'}} to = {'/creators/title/' + asset.id}><div id={asset.id} style = {{width: '100%'}}>{asset.name}</div></Link></td>
    ...
    deletePost(id){
        ...
        this.props.deleteCreatorMutation({
        ...
        return(
            <div className='main__show home__box all__box' id='testScroll' onScroll = { this.handleScroll }>
                <div className='home__main'>
                    ...
                    <Link to='/creators/add' style={style}><div className = 'new__blog'>
...
export default compose(
    ...
    graphql(getCreatorsQuery, {
        name: 'getCreatorsQuery',
        options: () => ({
            variables: {
                search: undefined, 
                first: undefined, 
    ...
    graphql(deleteCreatorMutation, {name: 'deleteCreatorMutation'})
